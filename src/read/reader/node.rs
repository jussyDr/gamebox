...
use crate::{
    read::{
        ...
        readable::{BodyChunks, BodyChunksInline, Class, ReadAbstract},
    },
    Error,
};
...
impl<R: Read, I: IdStateMut, N: NodeStateMut> Reader<R, I, N> {
    ...
    pub fn node<T: Default + Class + BodyChunks + 'static>(
        &mut self,
        ...
        let node = match slot {
            None => {
                ...
                let node: Rc<dyn Class> = Rc::new(node);
    ...
    /// Read a node of type `T`.
    pub fn node_abstract<T: ReadAbstract>(&mut self) -> Result<T, Error> {
        todo!()
    }
}
