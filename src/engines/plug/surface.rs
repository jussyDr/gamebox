...
mod read {
    ...
    use crate::{
        engines::plug::material::Material,
        read::{
            read_body_chunks,
    ...
    impl Surface {
        fn read_chunk_3(
            ...
            if surface_version != 2 {
                return Err(Error::new(ErrorKind::Unsupported("surface version")));
            }
            ...
            match surface_type {
                7 => {
                    ...
                    if version != 7 {
                        return Err(Error::new(ErrorKind::Unsupported("surface mesh version")));
                    }
                ...
                _ => return Err(Error::new(ErrorKind::Unsupported("surface type"))),
            }
