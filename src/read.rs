...
use crate::reader::{IdState, IdStateMut, NodeState, NodeStateMut, Reader, Result};

pub fn test(reader: impl Read + Seek) -> Result<()> {
    let mut reader = Reader::new(reader, (), ());

    if reader.byte_array()? != [b'G', b'B', b'X'] {
        todo!()
    }
    ...
    let mut r = reader.take(user_data_size as u64, (), ());
    ...
    for (chunk_id, chunk_size) in header_chunks {
        let mut r = r.take(chunk_size as u64, &mut id_state, ());

        match chunk_id {
            0x2e001003 => read_chunk_2e001003(&mut r)?,
            0x2e001006 => read_chunk_2e001006(&mut r)?,
            0x2e002000 => read_chunk_2e002000(&mut r)?,
            0x2e002001 => read_chunk_2e002001(&mut r)?,
            _ => todo!(),
        }
...
fn read_chunk_2e001003<R: Read, I: IdStateMut, N>(r: &mut Reader<R, I, N>) -> Result<()> {
    r.id_or_null()?; // null
    r.u32()?; // 26
...
fn read_chunk_2e001006<R: Read, I, N>(r: &mut Reader<R, I, N>) -> Result<()> {
    r.u32()?; // 0
    r.u32()?; // 0
...
fn read_chunk_2e002000<R: Read, I, N>(r: &mut Reader<R, I, N>) -> Result<()> {
    r.u32()?; // 1

    Ok(())
}

fn read_chunk_2e002001<R: Read, I, N>(r: &mut Reader<R, I, N>) -> Result<()> {
    r.u32()?; // 0

    Ok(())
}

fn read_chunk_2e001009<R: Read, I: IdStateMut, N>(r: &mut Reader<R, I, N>) -> Result<()> {
    r.string()?; // "Items"
    r.u32()?; // 0
    r.id_or_null()?; // null
...
fn read_chunk_2e00100b<R: Read, I: IdStateMut, N>(r: &mut Reader<R, I, N>) -> Result<()> {
    r.u32()?; // 0xffffffff
    r.u32()?; // 26
    r.id()?; // "r-brwiQCRnOZ2PIHcM0Q8A"
...
fn read_chunk_2e00201c<R: Read, I, N: NodeStateMut>(r: &mut Reader<R, I, N>) -> Result<()> {
    r.u32()?; // 5
    r.node(0x2e020000, |r| {
        loop {
            let chunk_id = r.u32()?;
...
fn read_chunk_2e020005<R: Read, I, N: NodeStateMut>(r: &mut Reader<R, I, N>) -> Result<()> {
    r.u32()?; // "piks"
    r.u32()?; // 52
    r.node(0x09187000, |r| {
        r.u32()?; // 10
...
fn read_chunk_09003003<R: Read, I, N: NodeStateMut>(r: &mut Reader<R, I, N>) -> Result<()> {
    r.u32()?; // 2
    r.list(|r| {
        r.u32()?; // 0
        r.node(0x090fd000, |r| {
...
fn read_chunk_09003005<R: Read, I: IdStateMut, N>(r: &mut Reader<R, I, N>) -> Result<()> {
    r.u32()?; // 0
    r.u32()?; // 1
    r.u32()?; // 0
    r.u32()?; // 2
